...
document.querySelectorAll(".message").forEach((button, index) => {
  button.addEventListener("click", () =>
    messageContainers[index].classList.add("hidden")
  );
});
...
function applyFilters() {
  ...
  document.querySelectorAll("tbody tr").forEach((row) => {
    ...
    const matchId =
      !values.id ||
      row.children[0].textContent.replace("#", "").trim() === values.id;
    const matchCliente =
      !values.cliente ||
      row.children[1].textContent
        .toLowerCase()
        .includes(values.cliente.toLowerCase());
    const matchStatus =
      !values.status || row.children[3].textContent.trim() === values.status;
  ...
  document.getElementById("filtro-vazio").style.display =
    linhasVisiveis === 0 && temFiltro ? "table-row" : "none";
