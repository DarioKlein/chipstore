...
$dal = new UsuarioDal();
$id = $_GET['id'];
if (isset($_SESSION['conteudo-editando-usuario-erro'])) {
  $usuarioParaEditar = $_SESSION['conteudo-editando-usuario-erro'];
} else {
  $usuarioParaEditar = $dal->findById($id);
}
?>
<!DOCTYPE html>
<html lang="pt-br">
...
<body class="flex flex-col lg:flex-row bg-[var(--main-bg-color)] text-white">
  ...
  <main class="flex-1 overflow-hidden p-6">
    ...
    <div class="flex flex-col gap-4 mt-4 bg-(--secondary-bg-color) rounded-2xl p-6 border border-gray-800">
      ...
      <?php
        unset($_SESSION['msg-erro-editando-usuario']);
      endif;
      ?>
      <?php if (isset($_SESSION['msg-erro-editando-usuario-email-invalido'])): ?>
        <div class="error-message-container flex gap-2 items-center justify-between bg-red-600/10 border border-red-600/50 rounded text-red-600 px-2 py-1">
          Email já registrado. Informe outro valor.
          <i class="error-message fa fa-times cursor-pointer p-1"></i>
        </div>
      <?php
        unset($_SESSION['msg-erro-editando-usuario-email-invalido']);
      endif;
      ?>
      ...
      <?php
      unset($_SESSION['conteudo-editando-usuario-erro']);
      ?>
    </div>
  </main>
