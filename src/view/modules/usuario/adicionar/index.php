...
if (isset($_SESSION['conteudo-usuario-erro'])) {
  $usuario = $_SESSION['conteudo-usuario-erro'];
}
?>
<!DOCTYPE html>
<html lang="pt-br">
  ...
  <main class="flex-1 overflow-hidden p-6">
    <header class="flex gap-4 items-center">
      ...
      <div>
        ...
        <h2 class="text-gray-400">Cadastre um novo usuário</h2>
      </div>
    </header>
    <div class="flex flex-col gap-4 mt-4 bg-(--secondary-bg-color) rounded-2xl p-6 border border-gray-800">
      <?php if (isset($_SESSION['msg-erro-criando-usuario'])): ?>
      ...
      <?php
        unset($_SESSION['msg-erro-criando-usuario']);
      endif;
      ?>
      <?php if (isset($_SESSION['msg-erro-criando-usuario-email-invalido'])): ?>
        <div class="error-message-container flex gap-2 items-center justify-between bg-red-600/10 border border-red-600/50 rounded text-red-600 px-2 py-1">
          Email já registrado. Informe outro valor.
          <i class="error-message fa fa-times cursor-pointer p-1"></i>
        </div>
      <?php
        unset($_SESSION['msg-erro-criando-usuario-email-invalido']);
      endif;
      ?>
      <form class="flex flex-col flex-wrap gap-8" method="POST" action="../../../actions/usuario/adicionar-usuario-action.php">
        ...
        <div class="flex justify-end items-center gap-3 flex-col-reverse sm:flex-row">
          ...
          <button type="submit" class="w-full sm:w-fit px-3 py-2 rounded-lg bg-(--main-color) hover:shadow-[0_0_7.5px_var(--main-color)] focus:shadow-[0_0_0_5px_var(--main-color-transparent)] transition-all text-(--secondary-bg-color)">Salvar Usuário</button>
        </div>
      </form>
      <?php
      unset($_SESSION['conteudo-usuario-erro']);
      ?>
    </div>
  </main>
